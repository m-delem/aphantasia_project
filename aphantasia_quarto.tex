...
règle logique change, et les ensembles ont aussi une difficulté
croissante. Les matrices ont pour objectif d'évaluer le raisonnement
abstrait, un versant non-verbal de l'intelligence fluide. Nous
...
aphantasiques\footnote{Cette hypothèse opérationnelle est celle qui se
  ...
  plus haut de \emph{wishful thinking}.}.
...
aphantasiques et les non aphantasiques : nous n'avons pas de littérature
antérieure pour justifier le fait que cette tâche mobilise des images
mentales ou non, et quelles composantes cognitives elle mobilise, nous
n'avons donc pas d'hypothèse définie sur la variabilité ici.
...
le document, leurs références ainsi que les liens vers le code détaillé
ici).
...
R via la fonction \texttt{factor\_analysis} du package \emph{parameters}
et \texttt{fviz\_pca\_var()} du package \emph{factoextra}.
...
écarts-types arbitraires et certains aspects approximatifs du fait des
contraintes techniques de notre simulation (les minimums et maximums par
...
Modèles graphiques gaussiens (GGM) Ces corrélations partielles peuvent
également être représentées par des modèles graphiques gaussiens (GGM),
un outil de plus en plus populaire en psychologie. Un GGM comprend
traditionnellement un ensemble de variables représentées par des cercles
(``nœuds''), et un ensemble de lignes qui visualisent les relations
entre elles, dont l'épaisseur représente la force de l'association (voir
Bhushan et al., 2019).

Nous avons évalué que les données étaient adaptées à une analyse
factorielle multiple avec plusieurs indices : - KMO : La mesure Kaiser,
Meyer, Olkin (KMO) de l'adéquation de l'échantillonnage suggère que les
données semblent appropriées à une analyse factorielle (KMO = 0,82). -
Sphéricité : Le test de sphéricité de Bartlett suggère qu'il y a une
corrélation significative suffisante dans les données pour une analyse
factorielle (Chisq(66) = 3194.04, p \textless{} .001).

ELISE « Cette analyse retient 3 dimensions présentant des valeurs
propres (ou eigenvalues) respectives de 4.02, 1.87 et 0.95. Ensuite, une
ACP avec une rotation oblimin a été conduite dont les résultats sont
présentés dans la Figure 1.7. Les trois dimensions de l'ACP réunissent
67.3\% de la variance totale. Les dimensions sont ensuite interprétées,
une valeur supérieure à .30 est considérée comme significative (Kline,
2002, pp.52-53). La première dimension était composée de la lecture de
pseudo-mots, la lecture de mots, la fluence en lecture, l'orthographe,
la conscience phonologique et la mémoire à court terme phonologique.
Nous avons par conséquent décidé de la nommer « Décodage et Phono ». La
seconde dimension était composée des mesures de vocabulaire, de
compréhension orale et de compréhension en lecture (CL). Cette dimension
a été appelée « Sémantique et CL ». La dernière dimension était composée
uniquement de la mesure de raisonnement non-verbal, et a donc été nommée
« Matrice ». »

Le choix de trois dimensions est soutenu par cinq (35,71\%) méthodes sur
quatorze (dont CNG, coordonnées optimales, analyse parallèle, critère de
Kaiser, Scree (SE)).\footnote{Nous avons par ailleurs mené une analyse à
  quatre facteurs, dans la mesure où ce nombre était le deuxième le plus
  ...
  Les 4 facteurs latents ont représenté 67,82\% de la variance totale
  des données originales (MR1 = 25,86\%, MR2 = 21,57\%, MR3 = 12,91\%,
  MR4 = 7,48\%).}. Nous avons donc conduit l'analyse avec la fonction
\texttt{factor\_analysis} du package \emph{parameters} sur R, réglée
pour trois facteurs et une rotation adaptée à l'analyse de clusters. La
