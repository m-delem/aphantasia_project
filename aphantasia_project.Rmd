...
```{r data_standardization, echo=TRUE}
data_scale <- data %>% 
  select(OSIQ_O:SRI) %>% 
...
```{r correlation_matrix, fig.width = 9, fig.cap="Les lunettes à Matrix"}
...
```{r network_graph,fig.width=9,fig.height=8,dpi=300, fig.cap="Network"}
data_scale %>%
  correlate() %>% 
  stretch %>% 
  filter(abs(r) >= .1) %>% 
...
```{r cluisters, fig.cap= "Clusters", out.width="75%"}
data_scale %>% 
