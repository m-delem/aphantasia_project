...
Violin plot vertical méga classe de Dawes pour afficher les scores des deux groupees sur tous les questionnaires

On peut ici observer les différences entre les deux groupes sur différents scores, avec des distributions qui se recoupent, et des scores équivalents à certains endroits


### Corrélations entre les variables mesurées

Comme nous l'avons vu, il existe de nombreux liens (ici par construction, à des fins d'observation). La \autoref{network_plot} en donne une représentation graphique explicite (voir \autoref{correlation_matrix} en \nameref{annexes} pour une matrice avec l'ensemble des *r* de corrélation et leur significativité). Parmi les covariances significatives, le VVIQ et l'OSIQ-objet sont extrêmement proches `test de corrélation`, le MRT, le SRI, l'OSIQ-spatial et les blocs de Corsi (toutes des tâches impliquant de l'imagerie spatiale) forment un réseau très corrélé (voir \autoref{correlation_matrix} pour les corrélations deux à deux), l'empan de chiffres est proche des blocs de Corsi (tâches mnésiques), la tâche de lecture est lointaine et 
