...
Comme nous l'avons vu, il existe de nombreux liens (ici par construction, à des fins d'observation). La \autoref{ggm_graph} en donne une représentation graphique explicite. 

Parmi les covariances significatives, le VVIQ et l'OSIQ-objet sont extrêmement proches, le MRT, le SRI, l'OSIQ-spatial et les blocs de Corsi (toutes des tâches impliquant de l'imagerie spatiale) forment un réseau très corrélé,l'empan de chiffres est proche des blocs de Corsi (tâches mnésiques) (Voir la \autoref{correlation_matrix} en \nameref{annexes} pour tous les coefficients de corrélation et leur significativité).
...
Ces corrélations partielles peuvent également être représentées par des modèles graphiques gaussiens (GGM),
un outil de plus en plus populaire en psychologie.
Un GGM comprend traditionnellement un ensemble de variables représentées par des cercles ("nœuds"),
et un ensemble de lignes qui visualisent les relations entre elles,
dont l'épaisseur représente la force de l'association (voir Bhushan et al., 2019).

Nous avons évalué que les données étaient adaptées à une analyse factorielle multiple avec plusieurs indices :
  - KMO : La mesure Kaiser, Meyer, Olkin (KMO) de l'adéquation de l'échantillonnage suggère que les données semblent appropriées à une analyse factorielle (KMO = 0,82).
  - Sphéricité : Le test de sphéricité de Bartlett suggère qu'il y a une corrélation significative suffisante dans les données pour une analyse factorielle (Chisq(66) = 3194.04, p < .001).
  
ELISE
« Cette analyse retient 3 dimensions présentant des valeurs propres (ou eigenvalues) respectives de 4.02, 1.87 et 0.95. Ensuite, une ACP avec une rotation oblimin a été conduite dont les résultats sont présentés dans la Figure 1.7. Les trois dimensions de l’ACP réunissent 67.3% de la variance totale. Les dimensions sont ensuite interprétées, une valeur supérieure à .30 est considérée comme significative (Kline, 2002, pp.52-53). La première dimension était composée de la lecture de pseudo-mots, la lecture de mots, la fluence en lecture, l’orthographe, la conscience phonologique et la mémoire à court terme phonologique. Nous avons par conséquent décidé de la nommer « Décodage et Phono ». La seconde dimension était composée des mesures de vocabulaire, de compréhension orale et de compréhension en lecture (CL). Cette dimension a été appelée « Sémantique et CL ». La dernière dimension était composée uniquement de la mesure de raisonnement non-verbal, et a donc été nommée « Matrice ». » 

Le choix de trois dimensions est soutenu par cinq (35,71%) méthodes sur quatorze (dont CNG, coordonnées optimales, analyse parallèle, critère de Kaiser, Scree (SE)).^[Nous avons par ailleurs mené une analyse à quatre facteurs, dans la mesure où ce nombre était le deuxième le plus supporté par les indices de détermination : la quatrième dimension identifiée par cette analyse rassemble quasi-exclusivement les scores au Corsi et à l'Empan Verbal, soit les deux tâches de mémoire de travail. Voir la \autoref{loadings_graph_annex} en \nameref{annexes}. Les 4 facteurs latents ont représenté 67,82% de la variance totale des données originales (MR1 = 25,86%, MR2 = 21,57%, MR3 = 12,91%, MR4 = 7,48%).]. Nous avons donc conduit l'analyse avec la fonction `factor_analysis` du package *parameters* sur R, réglée pour trois facteurs et une rotation adaptée à l'analyse de clusters. La \autoref{loadings_tbl} présente les capacités explicatives de chaque composante sur la variance de totale de l'échantillon. Les 3 facteurs latents ont représenté 62,55% de la variance totale des données originales (MR1 = 26,91%, MR2 = 21,53%, MR3 = 14,11%). La distribution des poids de chaque variable selon les facteurs est présentée en \autoref{loadings_graph}.
...
X% des aphantasiques sont dans un groupe performant
...
Dans l’objectif de mieux décrire les sous-groupes, nous réaliserons es analyses de variance univariées. 

Nous avons ajusté des modèles linéaires sur nos données pour prédire les nouveaux scores de *Raisonnement*, d'*Imagerie Objet* et d'*Imagerie Spatiale* en fonction des clusters, et ainsi mieux comparer et caractériser nos sous-groupes. Ces modèles ont permis d'expliquer une part de variance importante et statistiquement significative des scores en Raisonnement ($r^2$ = 0.49, *F*(3,
