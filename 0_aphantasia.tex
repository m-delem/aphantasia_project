...
raisonnement sont elles liées à de meilleures fonctions exécutives ?
...
cognitifs. Sur la base de l'étude de simulation et d'analyse
prévisionnelle réalisée précédemment, un plan analytique pourrait
consister à utiliser une méthode d'Analyse Factorielle Multiple et de
théorie des graphes pour identifier les facteurs cognitifs les plus
déterminants dans nos données et réduire nos variables, et dans un
second temps utiliser un algorithme de partition non-supervisée par la
méthode des \emph{k-means} pour étudier la répartition de nos
participants dans des groupes pertinents avec leurs profils cognitifs.
...
Source dédiée à la création de questionnaires. Les tâches seront codées
sur \href{https://osdoc.cogsci.nl/3.3/}{OpenSesame}
...
comportementales, et avec \href{https://www.jspsych.org/7.3/}{jspsych}
(\protect\hyperlink{ref-JsPsychJavaScriptLibrary}{\emph{{jsPsych}},
s.~d.}), une bibliothèque JavaScript OpenSource dédiée à la création
d'expériences. Avant les premiers questionnaires seront recueillies des
données démographiques (âge, genre, métier et/ou études, vision,
latéralité). Tous les participants devront donner leur consentement
éclairé avant de commencer l'étude. La participation sera volontaire et
sans compensation.
...
\emph{- À remplacer potentiellement par l'OSVIQ -}
...
al., 2017}) est un questionnaire évaluant plusieurs composantes de
l'imagerie visuospatiale, considérée comme un construit complexe en
quatre composantes, pour en avoir le reflet le plus fidèle possible. Il
inclut notamment des tests de rotation comme le MRT, mais aussi
d'orientation dans l'espace, de manipulation dans l'espace et de
visualisation.
...
Similitudes, aux textes de compréhension, au WCST, au SRI, l'empan
mnésique de chiffres, et le nombre de blocs rappelés au Corsi (qui
correspond à un empan mnésique spatial) - soit dix scores.
...
Les analyses ont été réalisées sur R (\protect\hyperlink{ref-R-base}{R
Core Team, 2022}).\\
Dans la littérature, les mesures de tâches comparables aux nôtres ont
...
et S.max le maximum. Dans le cas de la deuxième solution, nous pourrions
standardiser les scores en z-scores via la fonction \texttt{scale()} sur
R, puis les ramener sur une échelle de 0 à 1, avec une médiane à 0.5,
via la fonction \texttt{rescale()} du package \emph{datawizard} sur R -
de sorte à pouvoir construire des profils plus aisément.
...
Nos variables initiales (les scores) auront pour certaines des
corrélations très significatives entre elles du fait de la proximité des
capacités cognitives qu'elles évaluent. Nous pourrons alors réduire le
nombre de variables en isolant des dimensions essentielles qu'elles
représentent par une \textbf{Analyse Factorielle Multiple} (AFM).
Celle-ci nous permettra alors de combiner les scores de différentes
tâches pour les ramener à des scores prédictifs liés à des capacités
cognitives (e.g.~un score en imagerie visuelle au lieu de deux scores au
VVIQ et à l'OSIQ-objet). Cette analyse pourra être conduite sur R via la
fonction \texttt{factor\_analysis()} du package \emph{parameters} et
\texttt{fviz\_pca\_var()} du package \emph{factoextra}.

De même, nos groupes initiaux seront définis de manière arbitraire (une
...
corriger ce biais potentiel nous pourrions réaliser une \textbf{analyse
de partition non-supervisée} (dite en \emph{clusters}'') par
l'algorithme des \emph{k-means}, pour ainsi étudier la répartition en
groupes qu'il propose en prenant en compte toutes nos variables
redéfinies par l'AFM. L'algorithme fonctionne sur la base d'une
\emph{matrice de dissimilarité} selon des distances euclidiennes :
i.e.~il évalue ``géométriquement'' selon les axes de nos variables la
``distance'' entre chaque observation (ici les participants). La
distance euclidienne en deux dimensions se calcule simplement par le
théorème de Pythagore. Pour un nombre plus grand de dimensions, la
formule généralisée est la suivante :
...
via de nombreux indices. Nous pourrions ici utiliser la fonction
\texttt{n\_clusters()} du package \emph{parameters} sur R. L'analyse en
\emph{clusters} elle-même pourra être conduite avec la fonction
...
Pour finir, l'algorithme des \emph{k-means} pourrait donc permettre de
créer des groupes qui auront des profils particuliers sur chaque
composante cognitive représentée par nos variables. La composition de
ces groupes (en pourcentage d'aphantasiques/non-aphantasiques définis
initialement) ainsi que leurs capacités cognitives seraient analysées :
la variable du \emph{groupe} étant notre seule variable indépendante,
nous ajusterions alors des \textbf{modèles linéaires} sur nos données et
réaliserons des \textbf{ANOVAs univariées} en fonction du groupe ainsi
que des \textbf{t-tests post-hoc} (ou des ANOVAs non-paramétriques et
des tests de Mann-Whitney-U dans le cas de données
excessivement\footnote{Jugement à expliquer bien sûr.} non-normales).
...
\ldots{} En attente de données\ldots{}
...
\ldots{} En attente de données\ldots{} (et de la confirmation de la
pertinence de la démarche analytique\ldots)
...
\ldots{} Idem que sa parente l'AFM\ldots{}
...
\ldots{} S'entend si et seulement si des nouveaux clusters
apparaissent\ldots{} Sinon ce sont juste des ANOVAs univariées
paramétriques ou non sur des modèles linéaires avec les deux
groupes\ldots{}

\hypertarget{discussion}{%
\section{Discussion}\label{discussion}}

\ldots{} En attente de résultats\ldots{}
...
\leavevmode\vadjust pre{\hypertarget{ref-JsPsychJavaScriptLibrary}{}}%
\emph{{jsPsych}: {A JavaScript} Library for Creating Behavioral
Experiments in a {Web} Browser \textbar{} {SpringerLink}}. (s.~d.).
Consulté 14 février 2023, à l\textquotesingle adresse
...
repository de ce projet sur GitHub}}. Le code complet des questionnaires
et tâches créées est dans
\href{https://github.com/m-delem/aphantasia_experiment.git}{\emph{ce
repository associé au projet}}.

Les analyses ont été menées en langage statistique R (version 4.2.0 ; R
Core Team (\protect\hyperlink{ref-R-base}{2022})), sur Windows 10 x64
(build 19044), avec les packages suivants installés : \emph{bayestestR}
(version 0.13.0 ; Makowski, Lüdecke, et al.
